...
# [강제 수정] sqlite3 파일 잠금 문제 원천 차단
# Streamlit 재실행 시 중복 패칭 방지
if not hasattr(sqlite3, "_original_connect"):
    sqlite3._original_connect = sqlite3.connect

def patched_connect(*args, **kwargs):
    # yfinance 관련 DB 파일 접근 시 프로세스별 고유 파일 사용 (Lock 방지 + 스레드 공유)
    if args and isinstance(args[0], str) and ('yfinance' in args[0] or 'pyfin' in args[0]):
        temp_dir = tempfile.gettempdir()
        # 스레드별 파일 대신 프로세스별 파일 사용 (데이터 공유 문제 해결 시도)
        unique_name = f"yfinance_{os.getpid()}_shared.db"
        new_path = os.path.join(temp_dir, unique_name)
        
        # [중요] yfinance가 원본 DB 파일이 존재한다고 판단하면 테이블 초기화를 건너뛸 수 있음
        # 따라서 원본 DB가 존재하면, 새로운 임시 경로로 복사하여 테이블 구조를 유지해야 함
        if not os.path.exists(new_path) and os.path.exists(args[0]):
            try:
                shutil.copy2(args[0], new_path)
            except Exception:
                pass 
        ...
        if 'timeout' not in kwargs:
            kwargs['timeout'] = 30.0 # 타임아웃 증가
            
        return sqlite3._original_connect(new_path, **kwargs)
    return sqlite3._original_connect(*args, **kwargs)
...
# [버그수정] Windows 환경에서 한글 경로로 인한 SSL 인증서 오류 해결
def setup_ssl_environment():
    # 이 로직은 Windows 환경에서만 필요합니다. (Streamlit Cloud 등 Linux 환경에서는 실행하지 않음)
    if os.name != 'nt':
        return
    ...
    try:
        ...
        # 경로에 비아스키 문자(한글 등)가 포함되어 있는지 확인
        try:
            ca_bundle_path.encode('ascii')
            # ASCII 경로라면 문제 없음
            return
        except UnicodeEncodeError:
            pass # 비아스키 문자가 포함됨, 조치 필요

        # 안전한 경로(ASCII만 포함)로 복사
        # C:\Users\Public 은 일반적으로 쓰기 권한이 있고 ASCII 경로임
        ...
        # 파일이 없거나 원본과 다르면 복사
        if not os.path.exists(safe_path) or os.path.getsize(safe_path) != os.path.getsize(ca_bundle_path):
            try:
                shutil.copy2(ca_bundle_path, safe_path)
            except Exception:
                # 복사 실패 시 무시
                return
            
        # 환경 변수 설정 (yfinance/curl_cffi가 이를 사용하도록 함)
...
# [버그수정] yfinance 캐시 데이터베이스 잠금(disk I/O error) 문제 해결을 위한 캐시 삭제
def clear_yfinance_cache():
    try:
        import appdirs
        # yfinance가 사용하는 일반적인 캐시 경로들
        cache_dirs = [
            os.path.join(appdirs.user_cache_dir(), "py-yfinance"),
        ...
        for cache_dir in cache_dirs:
            if os.path.exists(cache_dir):
                try:
                    ...
                    print(f"Failed to clear cache at {cache_dir}: {e}")
    except Exception as e:
        print(f"Error clearing yfinance cache: {e}")
...
def fetch_dashboard_data():
    ...
    def process_rankings(ticker_list, period="5d"):
        ...
        try:
            ...
            st.warning(f"랭킹 데이터 다운로드 중 오류 발생 (Bulk): {e}")
            pass

        if data.empty:
            # st.info("개별 티커 데이터 다운로드 시도 중...")
            ...
            for sym in ticker_list:
                try:
                    s_data = None
                    # yfinance 0.2.x 이상에서는 MultiIndex 컬럼일 수 있음
                    if isinstance(data.columns, pd.MultiIndex):
                        try:
                            # 컬럼 레벨 확인 (Price, Ticker) 또는 (Ticker, Price)
                            # 보통 최신 버전은 (Price, Ticker) 구조임: Close -> Ticker
                            if 'Close' in data.columns.get_level_values(0):
                                s_data = data['Close'][sym].dropna()
                            elif sym in data.columns.get_level_values(0):
                                s_data = data[sym]['Close'].dropna()
                        except KeyError:
                             # Ticker가 레벨 1에 있는 경우 등 다양한 케이스 대응
                            pass
                    else:
        ...
        if not results:
            st.warning(f"수집된 랭킹 데이터가 없습니다. (대상: {len(ticker_list)}개 종목)")
...
def render_domestic_news(rankings):
    ...
    if items:
        ...
        st.info("뉴스 데이터를 불러올 수 없습니다. (종목별 뉴스 없음)")
...
def render_news_highlights(news_items):
    ...
    if news_items:
        ...
        st.info("해외 뉴스 하이라이트를 불러올 수 없습니다.")
...
def render_dashboard_section(data, key_prefix="kr"):
    ...
    if rankings:
        ...
        st.warning("랭킹 데이터를 불러올 수 없습니다. (데이터 수집 실패 또는 장 종료 후 데이터 지연)")
